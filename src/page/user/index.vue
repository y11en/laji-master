...
<div class="admin-user-wrap">
    ...
    <el-row class="mbt20">
        <el-table ref="multipleTable" :data="userList.list" border select="row" tooltip-effect="dark" style="width:100%">
            ...
            <el-table-column prop="userGoldenTicket" label="金椒"></el-table-column>

            <el-table-column prop="userReadTicket" label="阅读券"></el-table-column>

            <el-table-column prop="integration" label="积分"></el-table-column>

            <el-table-column prop="lastLogTime" label="最近登录">
                <template slot-scope="scope">
                    <span>{{scope.row.lastLogTime | time('long')}}</span>
                </template>
            </el-table-column>
            
            <el-table-column prop="userIpAddress" label="最新登录IP"></el-table-column>
            <el-table-column prop="city" label="登录地址"></el-table-column>
            ...
            <el-table-column prop="userQQ" label="QQ号码">
                <template slot-scope="scope">
                    <span>{{scope.row.userQQ === '0' ? '无' : scope.row.userQQ}}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column
                ...
                align="center">
                <template slot-scope="scope">
                    <router-link class="btn" :to="'/user/detail/'+scope.row.userId">编辑</router-link>
                    <el-dropdown size="medium" @command="handleCommand($event,scope.row)" trigger="click">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item command="g">一键登录</el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </template>
            </el-table-column>
    ...
    <PicCropper
        ref="avatarUpdate"
        action="https://www.lajixs.com/api/admin/updateUserAvatarimgUpload"
...
import PicCropper from '../../components/img_upload'
...
export default{
    ...
    methods: {
        ...
        getUserList() {
            ...
            this.$store.dispatch('user/getUserList', {params: searchValue, type: ''}).then(res => {
                if (res.returnCode === 200) {
                    this.IPCity(res.data)
        ...
        switch (com) {
            ...
            break
            case 'g':
                this.openLoginBox(row)
                break
        }
        },

        openLoginBox(row) {
            this.$confirm('确定登录？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                ...
                setTimeout(() => {
                    var encodeStr = Base64.encode(row.userPassword + "_LGADMIN")
                    window.location.href = 'https://www.lajixs.com/transfer?userName=' + row.userName + '&password=' + encodeStr
                }, 500)
