...
<div class="book-property-wrap">
    <el-alert title="操作说明" type="info" show-icon>
        <div>
            <p>您可以通过点击标签名或分类名来修改相关信息，也可以直接删除。</p>
            <p><span class="red">提示：</span>删除标签或分类会导致原来书籍标签或分类丢失，请谨慎使用！</p>
            <p><span>注意事项：</span>1.分类图标上传大图尺寸414X180、小图尺寸70X70，请严格按照尺寸要求操作</p>
        </div>
    </el-alert>
      
    <el-row class="mbt20 book-property-list" v-loading="this.$store.state.book.loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading">
        <h2 class="tab-title">书籍标签</h2>
        <el-button
            v-for="item in editBookEchoData.booklablesList"
            :key="item.id"
    ...
    <el-row class="mbt20 book-property-list" v-loading="this.$store.state.book.loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading">
        ...
        <el-button
            v-for="item in editBookEchoData.classificationList"
            :key="item.id"
            size="small"
            type="primary"
            plain
...
export default {
    ...
    mounted() {
        this.$store.dispatch('book/editBookEcho')
    },
    computed: {
        editBookEchoData() {
            return this.$store.state.book.editBookEchoData
        }
    },
    methods: {
