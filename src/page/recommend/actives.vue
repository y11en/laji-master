...
<div class="book-recommend-actives">
    ...
    <el-row :gutter="10" class="mbt20">
        <el-col :xs="18" :sm="12" :md="9" :lg="6" :xl="3"></el-col>
        <el-button class="fr" @click="$clearCache()">清除缓存</el-button>
    </el-row>

    <el-row v-loading="this.$store.state.system.loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading">
        <el-table ref="multipleTable" :data="tableList" border select="row" tooltip-effect="dark" style="width: 100%">
            <el-table-column type="selection" align="center"></el-table-column>

            <el-table-column prop="id" label="Id" width="34" align="center"></el-table-column>

            <el-table-column prop="type" label="终端" width="50" align="center">
                <template slot-scope="scope">
                    {{scope.row.type ? 'PC' : 'App'}}
                </template>
            </el-table-column>
    
            <el-table-column label="小图" align="center" minWidth="300">
                <template slot-scope="scope">
                    <img class="mw-auto" :src="scope.row.activityImgURL" alt="" @click="handleClick(scope.row,'small')">
                </template>
            </el-table-column>
            
            <el-table-column label="大图" minWidth="400" align="center">
                <template slot-scope="scope">
            ...
            <el-table-column label="详情链接" align="center">
                <template slot-scope="scope">
                    <el-dropdown size="medium" @command="handleCommand(scope.row)" trigger="click">
...
export default{
    ...
    created() {
        this.$store.dispatch('system/getActivityRecommendedPosition')
    },
    computed: {
        tableList() {
            return this.$store.state.system.activeRecommendData
        }
    },
    methods: {
        // getRecommendList() {
        ...
        handleCommand(row) {
            this.$store.dispatch('system/HDupdateActivityRecommendedPosition', { id: row.id, showHide: row.showHide ? 0 : 1 })
        ...
        editActiveRecommend() {
            const subData = () => {
                ...
                this.$store.dispatch('system/HDupdateActivityRecommendedPosition', this.formValue)
