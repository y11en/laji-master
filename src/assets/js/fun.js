...
exports.install = function(Vue, options) {
  ...
  Vue.prototype.$formTime = function(date, type, separator) {
    ...
    const now = new Date(date).getFullYear() || new Date().getFullYear()
    const days = 0
    let Time, T
    T = time
    const year = T.getFullYear()
    const mon = T.getMonth() + 1 < 10 ? '0' + (T.getMonth() + 1) : T.getMonth() + 1
    const Date1 = T.getDate() < 10 ? '0' + T.getDate() : T.getDate()
    ...
    this.$consume({
      ...
      beforeClose: (action, instance, done) => {
        if (action === 'confirm' && type === 'letter') {
          const len = this.$http.trim(instance.messageContent).length
          if (len < 200 && len > 0) {
